...
def parse_stats_from_eu_match(
    ...
    ps_before_ot = {p.name: {**stat_defaults} for p in m.players}
    snapshotted = False
    score_before_ot = (0, 0)
    total_score = (0, 0)
            ...
            for p2 in ps.values():
                if p2["team"] is not None:
                    if p2["team"] == p["team"]:
                        p2["caps_for"] += 1
                        if (
                            p2["last_return_time"] is not None
                            and time - p2["last_return_time"] < 2 * 60
                        ):
                            p2["key_returns"] += 1
                    else:
                        p2["caps_against"] += 1
                ...
                if p2["team"] != p["team"] and p2["last_hold_end"] == time:
                    ...
                    if _dist(sx, sy, *enemy_flag) < 6:
                        own_team_with_flag = [
                            p3
                            for p3 in ps.values()
                            if p3["grab_time"] is not None
                            and p3["last_hold_end"] is None
                        ]
...
def process_game_stats(game: Game):
    ...
    players = {p.playing_as: p for p in PlayerGameLog.objects.filter(game=game)}
    m, m2 = None, None
    ...
    ps, ps_before_ot, team_mapping, score_before_ot, total_score = (
        parse_stats_from_eu_match(m, game.paused_time or 600)
    )
    ...
    if game.resumed_tagpro_eu:
        ps2, ps2_before_ot, team_mapping2, score2_before_ot, total_score2 = (
            parse_stats_from_eu_match(
                m2, stats_count_until=game.resumed_stats_count_until or 0
            )
        )
        ...
        if _is_half_or_ot_period(game.game_in_match):
            ...
            combined_total = (
                ...
                total_score[1] + total_score2[1],
            )
            combined_before_ot = (
                score_before_ot[0] + score2_before_ot[0],
                score_before_ot[1] + score2_before_ot[1],
            )
            ...
            if is_ot_period:
                game.team1_score += (
                    m2.team_red.score if team1_is_red else m2.team_blue.score
                )
                game.team2_score += (
                    m2.team_blue.score if team1_is_red else m2.team_red.score
                )
            else:
            ...
            went_to_ot = (
                is_ot_period
                or score_before_ot[0] + score2_before_ot[0]
                == score_before_ot[1] + score2_before_ot[1]
            )
            game.had_ot = went_to_ot
    ...
    for p in players:
        ...
        player_stat_defaults = {stat: ps[p][stat] for stat in STAT_FIELDS}
        player_regulation_stat_defaults = {
            stat: ps_before_ot[p][stat] for stat in STAT_FIELDS
        }
        game_stats, _ = PlayerStats.objects.update_or_create(
            player_gamelog=players[p], defaults=player_stat_defaults
        )
        regulation_game_stats, _ = PlayerRegulationStats.objects.update_or_create(
            player_gamelog=players[p], defaults=player_regulation_stat_defaults
        )
        game_stats.save()
...
def aggregate_stats(pgs: models.QuerySet[PlayerStats]) -> Dict[str, int]:
    ...
    """
    aggregate_fields = {f"{field}_sum": models.Sum(field) for field in STAT_FIELDS}
    totals = pgs.aggregate(**aggregate_fields)
    return {key.replace("_sum", ""): value for key, value in totals.items()}
...
def rank_by_standing_points(teams_data):
    ...
    while i < len(teams_data):
        ...
        tied_group = []
        while (
            i < len(teams_data) and teams_data[i]["standing_points"] == current_points
        ):
    ...
    for team_data in teams_data:
        ...
        for opp_id in tied_team_ids:
            if opp_id in team_data["head_to_head"]:
                team_h2h_points += team_data["head_to_head"][opp_id][
                    "team_standing_points"
                ]
                total_h2h_points += team_data["head_to_head"][opp_id][
...
def rank_by_common_opponents_record(teams_data):
    ...
    for team_data in teams_data:
        ...
        for opp_id in common_opponents:
            common_team_points += team_data["head_to_head"][opp_id][
                "team_standing_points"
            ]
            common_total_points += team_data["head_to_head"][opp_id][
                "total_standing_points"
            ]

        common_win_pct = (
            common_team_points / common_total_points if common_total_points > 0 else 0
        )
...
def rank_by_common_opponents_cap_diff(teams_data):
    ...
    for team_data in teams_data:
        common_caps_for = sum(
            team_data["head_to_head"][opp_id]["caps_for"] for opp_id in common_opponents
        )
        common_caps_against = sum(
            team_data["head_to_head"][opp_id]["caps_against"]
            for opp_id in common_opponents
        )
...
def update_standings(season: Season):
    ...
    for team in teams:
        ...
        head_to_head = (
            {}
        )  # opponent_id -> {'team_standing_points': int, 'total_standing_points': int, 'caps_for': int, 'caps_against': int}
        ...
        for game in team_games:
            ...
            if opponent.id not in head_to_head:
                head_to_head[opponent.id] = {
                    "team_standing_points": 0,
                    "total_standing_points": 0,
                    "caps_for": 0,
                    "caps_against": 0,
                }
            ...
            h2h["total_standing_points"] += (
                team_standing_points + opponent_standing_points
            )
    ...
    for i, team_data in enumerate(standings_data):
        ...
        if not has_playoffs:
            ...
            playoff_matches = (
                Match.objects.filter(season=season, playoff_series__isnull=False)
                .filter(models.Q(team1=team) | models.Q(team2=team))
                .order_by("date")
            )
...
def calculate_scar(season: Season):
    ...
    if league_minutes_total > 0:
        ...
        for stat in regulation_stats:
            ...
            stat._oscar_normalized = stat._oscar_ba - (
                league_oscar_per_minute * adjusted_minutes
            )
            stat._dscar_normalized = stat._dscar_ba - (
                league_dscar_per_minute * adjusted_minutes
            )
    ...
    for game_id, game_stats in stats_by_game.items():
        ...
        team1_adjustment_per_minute = (
            team1_adjustment_total / team1_minutes_total
            if team1_minutes_total > 0
            else 0
        )
        team2_adjustment_per_minute = (
            team2_adjustment_total / team2_minutes_total
            if team2_minutes_total > 0
            else 0
        )
        ...
        for stat in game_stats:
            ...
            adjustment_per_minute = (
                team1_adjustment_per_minute if is_team1 else team2_adjustment_per_minute
            )
            adjusted_minutes = stat._time_played_minutes * stat._blowout_multiplier
...
def infer_playoff_series(season: Season):
    ...
    for m in playoff_matches:
        ...
        team1_prev_series = (
            PlayoffSeries.objects.filter(match__season=season, match__date__lt=m.date)
            .filter(models.Q(match__team1=m.team1) | models.Q(match__team2=m.team1))
        ...
        team2_prev_series = (
            PlayoffSeries.objects.filter(match__season=season, match__date__lt=m.date)
            .filter(models.Q(match__team1=m.team2) | models.Q(match__team2=m.team2))
            .order_by("-match__date")
            .first()
        )
        ...
        if (
            team1_prev_series
            and team1_prev_series.winner
            and team1_prev_series.winner != m.team1
        ):
            team1_prev_series = None

        if (
            team2_prev_series
            and team2_prev_series.winner
            and team2_prev_series.winner != m.team2
        ):
