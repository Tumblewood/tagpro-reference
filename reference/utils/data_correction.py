...
def merge_player_seasons(
    to_merge: PlayerSeason, target: PlayerSeason, cleanup_player=True
):
    """
    ...
    if (
        cleanup_player
        and not PlayerSeason.objects.filter(player=player_to_check).exists()
    ):
...
def merge_players(to_merge: Player, target: Player):
    ...
    for ps_to_merge in player_seasons_to_merge:
        ...
        existing_target_ps = PlayerSeason.objects.filter(
            player=target, season=ps_to_merge.season
        ).first()

        if existing_target_ps:
            # Merge the two player seasons (don't cleanup player yet, we'll do that at the end)
            print(
                f"Merging player seasons in {ps_to_merge.season.name}: {ps_to_merge} -> {existing_target_ps}"
            )
