...
def load_tpl_api_links():
    ...
    data_file = Path(__file__).parent.parent / "bulk_import_jsons" / "tpl_api_links.json"
...
def create_bulk_json_for_season(tpl_data, league_name, season_number, all_matches, team_abbrs):
    """Create bulk import JSON for a specific league and season"""
    
    # Filter games for this specific league and season
    season_games = [
        game for game in tpl_data['games'] 
    ...
    for game_info in season_games:
        match_key = f"Week {game_info['week']} {game_info['team1']} vs {game_info['team2']}"
    ...
    for game_info in season_games:
        ...
        if not has_tagpro_eu_link(game_info.get('links', [])):
            print(f"Skipping game without tagpro.eu: {league_name} S{season_number}, Week {game_info['week']}")
            continue
        ...
        try:
            ...
            # Infer teams
            red_team = infer_team(season_group, game_info['team1']) if season_group else None
            blue_team = infer_team(season_group, game_info['team2']) if season_group else None
            ...
            season_obj = red_team.season if red_team else (blue_team.season if blue_team else None)
            ...
            if season_obj:
                ...
                if team1_key not in team_seasons:
                    ...
                    team1_abbr = red_team.abbr if red_team else team_abbrs.get(team1_name, game_info['team1'])
                    
                    team_seasons[team1_key] = {
                        'season': season_obj.name,
                        'franchise': red_team.franchise.name if red_team and red_team.franchise else team1_name,
                        'name': team1_name,
                        'abbr': team1_abbr
                    }
                ...
                if team2_key not in team_seasons:
                    # Get abbreviation from our loaded team data
                    team2_abbr = blue_team.abbr if blue_team else team_abbrs.get(team2_name, game_info['team2'])
                    
                    team_seasons[team2_key] = {
                        'season': season_obj.name,
                        'franchise': blue_team.franchise.name if blue_team and blue_team.franchise else team2_name,
                        'name': team2_name,
                        'abbr': team2_abbr
                    }
            
            # Create match entry
            match_key = f"{season_name} Week {game_info['week']} {team1_name} vs {team2_name}"
            ...
            current_match_key = f"Week {game_info['week']} {game_info['team1']} vs {game_info['team2']}"
            games_for_this_match = matches_games[current_match_key]
            game_in_match = get_game_in_match_name(game_info['game'], game_info.get('half'), games_for_this_match)
            
            game_entry = {
                'game_in_match': game_in_match,
                'tagpro_eu': int(tagpro_eu_id),
                'map_name': game_data.get('map_name'),
                'red_team': team1_name if game_data['team_red']['name'] == game_info['team1'] or not red_team else team2_name,
                'blue_team': team2_name if game_data['team_blue']['name'] == game_info['team2'] or not blue_team else team1_name,
                'team1_score': game_data['team_red']['score'] if game_data['team_red']['name'] == game_info['team1'] or not red_team else game_data['team_blue']['score'],
                'team2_score': game_data['team_blue']['score'] if game_data['team_blue']['name'] == game_info['team2'] or not blue_team else game_data['team_red']['score'],
            ...
            for player_data in game_data['players']:
                ...
                if player and season_obj:
                    player_key = f"{season_obj.name} {player.name} {player_data['username']}"
                    if player_key not in player_seasons:
                        player_seasons[player_key] = {
                            'season': season_obj.name,
                            'player': player.name,
                            'playing_as': player_data['username'],
                            'team': team1_name if player_data['team'] == 'red' and game_entry['red_team'] == team1_name else (
                                team2_name if player_data['team'] == 'blue' and game_entry['blue_team'] == team2_name else 
                                team1_name if player_data['team'] == 'red' else team2_name
                            )
                        }
...
def main():
    ...
    if not all_matches:
        print("No matches loaded! Please ensure league_matches*.json and league_maps.json files exist in the data directory.")
        return
    ...
    for league, season in seasons:
        json_data = create_bulk_json_for_season(tpl_data, league, season, all_matches, team_abbrs)
        ...
        if json_data:
            ...
            print(f"Created {filename} with {len(json_data['team_seasons'])} teams, {len(json_data['player_seasons'])} player seasons, {len(json_data['matches'])} matches")
